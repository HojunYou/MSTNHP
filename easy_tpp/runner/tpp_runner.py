...
class TPPRunner(Runner):
    ...
    def run_one_epoch(self, data_loader, phase):
        ...
        # if phase in [RunnerPhase.TRAIN, RunnerPhase.VALIDATE]: ### HJ
        if phase in [RunnerPhase.TRAIN, RunnerPhase.VALIDATE, RunnerPhase.PREDICT]:
            ...
            ### HJ: clear up gradients
            self.model_wrapper.opt.step()
            self.model_wrapper.opt.zero_grad(set_to_none=True)
            ...
            ### HJ
            for batch_idx, batch in enumerate(data_loader):
                batch_pred, batch_label = self.model_wrapper.run_batch(batch, batch_idx=batch_idx, phase=phase, accumulation_steps=2)
                epoch_pred.append(batch_pred)
                epoch_label.append(batch_label)
            ### HJ: clear up gradients
            self.model_wrapper.opt.step()
