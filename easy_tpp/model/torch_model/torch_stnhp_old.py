...
class ContTimeSpaceLSTMCell(nn.Module):
    ...
    def init_dense_layer(self, hidden_dim, bias, beta):
        ...
        self.layer_decay = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim * (self.n_comps+1), bias=bias), #n_comps from spatial and 1 from time component
    ...
    def forward(self, x_i, hidden_i_minus, cell_i_minus, cell_bar_i_minus):
        ...
        gate_decay = gate_decay.view(-1, self.hidden_dim, self.n_comps+1) # reshape gate_decay to [batch_size, hidden_dim, 2] (for isotropic case)
    ...
    def seq_decay(self, cell_i, cell_bar_i, gate_decay, gate_output, dtime, dspace):

        dx = torch.cat((dtime, dspace), dim=-1)
        c_t = cell_bar_i + (cell_i - cell_bar_i) * torch.exp(-gate_decay*dx[...,None, :].sum(-1)).squeeze(-1)
        h_t = gate_output*torch.tanh(c_t)
...
class STNHP(TorchBaseModel):
        ...
        if max_seq_length == 1:
            ...
            for i in range(max_seq_length):
                if i == event_seq.size(1) - 1:
                    ...
                    ds = torch.ones_like(space_delta_seq[:, i]) * max_decay_space
                elif i==0:
                    dt = time_delta_seq[:, i + 1]
                    ds = space_delta_seq[:, 1]
                    ds = torch.norm(ds, p=2, dim=-1, keepdim=True)
                    #append ds[:,:2] to be zeros
                    ds = torch.cat([torch.zeros_like(ds[:,:1])]*(self.n_comps-1)+[ds], dim=1)
                    
                elif i<=self.n_comps-2:
                    dt = time_delta_seq[:, i + 1]
                    ds = space_seq[:,(i+1):(i+2)]-space_seq[:,:(i+1)]
                    ...
                    ds = space_seq[:,(i+1):(i+2)]-space_seq[:,(i+1-(self.n_comps)):(i+1)]
                    ds = torch.norm(ds, p=2, dim=-1)
    ...
    def loglike_loss(self, batch):
        ...
        # interval_s_sample = self.make_dspace_loss_samples(space_seqs[:, :-1])
        # interval_st_sample = self.make_dtimespace_loss_samples(time_delta_seqs[:,1:], space_seqs[:,:-1])
        ...
        # interval_s_sample = torch.cat([torch.zeros_like(interval_s_sample), torch.zeros_like(interval_s_sample), interval_s_sample], dim=-1)
        # interval_s_sample[:,:,2:,0] = interval_s_sample[:,:,:-2,2]
        # interval_s_sample[:,:,1:,1] = interval_s_sample[:,:,:-1,2]
    ...
    def make_multiple_dspace_loss_samples(self, space_seq):
        ...
        # zero1 = torch.zeros(B,1).to(space_seq.device)
        # zero2 = torch.zeros(B,2).to(space_seq.device)
        # space_xlower_limits = torch.cat([
        #     torch.cat([zero2, space_xlower_limit_single[:,:-2]], dim=-1)[...,None],
        #     torch.cat([zero1, space_xlower_limit_single[:,:-1]], dim=-1)[...,None],
        #     space_xlower_limit_single[...,None]
        # ], dim=-1)
        # space_xupper_limits = torch.cat([
        #     torch.cat([zero2, space_xupper_limit_single[:,:-2]], dim=-1)[...,None],
        #     torch.cat([zero1, space_xupper_limit_single[:,:-1]], dim=-1)[...,None],
        #     space_xupper_limit_single[...,None]
        # ], dim=-1)
        # space_ylower_limits = torch.cat([
        #     torch.cat([zero2, space_ylower_limit_single[:,:-2]], dim=-1)[...,None],
        #     torch.cat([zero1, space_ylower_limit_single[:,:-1]], dim=-1)[...,None],
        #     space_ylower_limit_single[...,None]
        # ], dim=-1)
        # space_yupper_limits = torch.cat([
        #     torch.cat([zero2, space_yupper_limit_single[:,:-2]], dim=-1)[...,None],
        #     torch.cat([zero1, space_yupper_limit_single[:,:-1]], dim=-1)[...,None],
        #     space_yupper_limit_single[...,None]
        # ], dim=-1)
        ...
        sampled_dspace = torch.stack([multiple_cartesian(x_coord, y_coord) for x_coord, y_coord in zip(x_coords, y_coords)], dim=0).to(space_seq.device)
    ...
    def make_multiple_dtimespace_loss_samples(self, time_delta_seq, space_seq):
        ...
        for i in range(space_samples.shape[2]):
            ...
            st_sample = st_sample.to(time_samples.device)
    ...
    def _make_equal_3dsamples(self, time_seq, time_sample, space_sample):
        ...
        for i, time_idx in enumerate(time_order[:-1]):
            ...
            st_samples.append(st_sample)
        # st_samples = torch.cat(st_samples, dim=0)
        return st_samples

    def make_equal_dtimespace_samples(self, time_seqs, time_domain, space_seqs, num_sample_t = 500):

        start_times = time_seqs[:,0]
        time_samples = torch.linspace(time_domain[0]+(time_domain[1]-time_domain[0])/num_sample_t, time_domain[1], num_sample_t)
        ...
        # space_samples = model_runner.model_wrapper.model.make_dspace_loss_samples(space_seqs)
    ...
    def make_multiple_equal_dtimespace_samples(self, time_seqs, time_domain, space_seqs, num_sample_t = 500):
        ...
        # space_samples = model_runner.model_wrapper.model.make_multiple_dspace_loss_samples(space_seqs)
        time_sample_list = [time_samples[start_time<time_samples] for start_time in start_times]
        ...
        # st_equal_sample0 = [self._make_equal_3dsamples(time_seq, time_sample, space_sample) for time_seq, time_sample, space_sample in zip(time_seqs, time_sample_list, space_samples[:,:,:,0,:])]
        # st_equal_sample1 = [self._make_equal_3dsamples(time_seq, time_sample, space_sample) for time_seq, time_sample, space_sample in zip(time_seqs, time_sample_list, space_samples[:,:,:,1,:])]
        # st_equal_sample2 = [self._make_equal_3dsamples(time_seq, time_sample, space_sample) for time_seq, time_sample, space_sample in zip(time_seqs, time_sample_list, space_samples[:,:,:,2,:])]
        ...
        # st_equal_sample_list= []
        # for i in range(len(st_equal_sample0)):
        #     assert len(st_equal_sample0[i]) == len(st_equal_sample1[i]) == len(st_equal_sample2[i])
        #     st_sample0, st_sample1, st_sample2 = st_equal_sample0[i], st_equal_sample1[i], st_equal_sample2[i]
        #     st_sample_list = []
        #     for st0, st1, st2 in zip(st_sample0, st_sample1, st_sample2):
        #         st_sample_list.append(torch.stack([st0, st1, st2], dim=1))
